...
class MO_Custom_OAuth1{
    ...
    {	
        ...
        $oauth1_getrequest_object = new MO_Custom_OAuth1_Flow($client_id, $client_secret, $request_token_url, $access_token_url, $userinfo_url); 
    ...
    static function mo_oidc1_get_access_token($appname)
    {
        ...
        $mo_oauth1_getaccesstoken_object = new MO_Custom_OAuth1_Flow($client_id,$client_secret,$request_token_url,$access_token_url,$userinfo_url);
        ...
        foreach ($response_parse as $key) {
        	...
        	else if($arg_parse[0] == 'oauth_token_secret'){
        		$oa_secret = $arg_parse[1];
        	}
        }
    
        $mo_oauth1_get_profile_signature_object = new MO_Custom_OAuth1_Flow($client_id,$client_secret,$request_token_url,$access_token_url,$userinfo_url);
...
class MO_Custom_OAuth1_Flow{
    ...
    function mo_oauth1_get_access_token($oauth_verifier, $mo_oauth1_oauth_token)
    {
        ...
        $mo_ts = isset($_COOKIE['mo_ts'])?$_COOKIE['mo_ts']:'';
    ...
    function mo_oauth1_https($url, $post_data = null){
        ...
        if ( is_wp_error( $get_response ) ) {
            MO_Oauth_Debug::mo_oauth_log('Invalid response recieved. Please contact your administrator for more information.');
            MO_Oauth_Debug::mo_oauth_log($get_response);
            wp_die( $response );
        }
    ...
    {
        if (is_array($input)) {
            return array_map(array('MO_Custom_OAuth1_Flow', 'mo_oauth1_url_encode_rfc3986'), $input);
        }
