...
class Plugin {
	...
	public function adminNotices( $show_on_demand = false ) {
        ...
        <div id="ppscc_birthday_notice" class="notice notice-success is-dismissible">
            <p><span class="dashicons dashicons-buddicons-groups"
                ...
                <strong><?php echo sprintf( __( 'Hello %s!', 'postpage-specific-custom-css' ), esc_html( $current_user->display_name ) ); ?></strong>
            </p>
            <p>
				<?php
				if ( $this->isBirthday ) {
            ...
            <p style="text-align: center">
				<?php foreach ( $flagList as $code ) : ?>
                    <a href="#" class="ppsc_show"
                       data-ppsc="<?php echo esc_attr( $code ); ?>"><?php echo strip_tags( $this->flag( $code ), '<img>' ); ?></a>
				<?php endforeach; ?>
            </p>
			<?php
			$accounts = [
	...
	public function accountLine( $code, $account, $currency ) {
		?>
        <div class="ppsc_hide_all" id="acinfofor_<?php echo esc_attr( $code ); ?>">
